...
void PutCharReversed( uint16_t Xpos, uint16_t Ypos, uint8_t ASCI, uint16_t charColor, uint16_t bkColor )
{
	uint16_t i, j;
    uint8_t buffer[16], tmp_char;
    GetASCIICode(buffer,ASCI);  /* 取字模数据 */
...
void GUI_Text_reversed(uint16_t Xpos, uint16_t Ypos, uint8_t *str,uint16_t Color, uint16_t bkColor)
{
    uint8_t TempChar;
    do
    {
        TempChar = *str++;  
        PutCharReversed( Xpos, Ypos, TempChar, Color, bkColor );    
        if( Xpos < MAX_X - 8 )
        {
            Xpos += 8;
        } 
...
void LCD_Bordi(int width)            //inizializzazione dei bordi
	{
int i;
	int altezza_bordo;
	altezza_bordo=32;
for(i=0;i<width+1;i++)
	{
	LCD_DrawLine(i,0,i,320-altezza_bordo,Red);
	LCD_DrawLine(240-i,0,240-i,320-altezza_bordo,Red);
	}
return;
}

void LCD_Paddle(int width,int thick,int startx,int starty,int numpaddle)  //inizializzazione del paddle
{
int i;
  //salvo le variabili che identificano il paddle
	if(numpaddle==1)
	{
	toprightx=startx+width;
	toprighty=starty;
	bottomleftx=startx;
	bottomlefty=starty+thick;
	}
	else if(numpaddle==2)
	{
	toprightx2=startx+width;
	toprighty2=starty;
	bottomleftx2=startx;
	bottomlefty2=starty+thick;
	
	}
	
	for(i=startx; i<startx+width+1;i++)
	{
	LCD_DrawLine(i,starty,i,starty+thick,Green);
	}
...
void LCD_Ball(int startx,int starty)
{
int i;
	for(i=startx-2;i<=startx+2;i++)
	{
		LCD_DrawLine(i,starty-2,i,starty+2,Blue);
	}
	x_ball=startx;
	y_ball=starty;
...
void LCD_EraseBall(int startx,int starty)
{
int i;
	for(i=startx-2;i<=startx+2;i++)
	{
		LCD_DrawLine(i,starty-2,i,starty+2,Black);
	}
	return;
